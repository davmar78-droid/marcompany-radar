#!/bin/bash
cat > ~/Desktop/trading_bot/bot.py << 'ENDOFBOT'
import requests
import pandas as pd
                ...
                try:
                    ...
                    if signal:
                        ...
                        if should_send(symbol,tf):
                            send_whatsapp(build_message(signal))
                            save_signal(signal)
...
if __name__=="__main__":
    run()
ENDOFBOT
echo "bot.py creado: $(wc -c < ~/Desktop/trading_bot/bot.py) bytes"
