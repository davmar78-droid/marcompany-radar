...
import numpy as np, json, time, logging, sqlite3, urllib.parse
...
def send_telegram(message):
    ...
    for chat in chats:
        try:
            import time
            time.sleep(0.5)  # Delay entre envíos
...
def send_telegram(message):
    phone=CONFIG["telegram_token"]; apikey=CONFIG["telegram_chats"]
...
def run():
    last_signal_time = {}  # Reset cooldown
    log.info("🚀 MARCO TRADING BOT FINAL INICIADO")
    send_telegram("🤖 Marco Trading Bot Final iniciado\n📊 Análisis técnico completo + Telegram\n✅ Monitorizando: " + ", ".join(CONFIG["symbols"]))
    init_db()
        ...
        for symbol in CONFIG["symbols"]:
            for tf in CONFIG["timeframes"]:
                try:
                    ...
                    signal = analyze(df, symbol, tf)
                    if signal:
                        log.info(f"🚨 SEÑAL {signal['direction']} {symbol} {tf} R/R:{signal['rr_tp2']}")
                        if should_send(symbol, tf):
                            log.info(f"📤 Enviando señal {symbol} {tf} (cooldown OK)")
                            send_telegram(build_message(signal))
                            save_signal(signal)
                            last_signal_time[f"{symbol}_{tf}"] = datetime.now(timezone.utc)
                        else:
                            log.info(f"⏸️ Señal {symbol} {tf} bloqueada por cooldown")
                    else:
